...
def get_groq_keys() -> list:
    ...
    if settings.groq_api_key_1:
        keys.append(settings.groq_api_key_1)
...
def call_groq_with_fallback(prompt: str, system_content: str, model: str = "llama3-70b-8192", temperature: float = 0.0) -> str:
    keys = get_groq_keys()
    if not keys:
    ...
    for idx, key in enumerate(keys):
        try:
            logger.info(f"Attempting Groq call with key index {idx}...")
            ...
            logger.warning(f"Groq call failed with key index {idx}: {e}")
            last_err = e
...
"""
    try:
        response_text = call_groq_with_fallback(
            prompt=prompt,
            system_content="You are a precise JSON extractor that only outputs raw JSON arrays."
        )
...
"""
    try:
        response_text = call_groq_with_fallback(
            prompt=prompt,
            system_content="You are a precise recruiter assistant that only outputs raw JSON objects."
        )
        cleaned = clean_json_response(response_text)
        return json.loads(cleaned)
    except Exception as e:
